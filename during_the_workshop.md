...
Write an integration test in test_app.py that uses the client fixture to send a request to the AccountsResource POST endpoint (i.e. a POST request to `/accounts/<account name here>`). Check the response status code and body to ensure the item was created correctly.

Expand your integration test to also check you can query the created account via the AccountsResource GET endpoint (i.e. a GET request to the same path)
